...
import { isForkKeptAgent } from '../../../shared/fork-config'
import {
  AgentLetterIcon,
  AiderIcon,
  CopilotIcon,
...
export const getAgentCatalog = createLocalizedCatalog((): AgentCatalogEntry[] => {
  // Why: fork only surfaces kept agents — filter the full catalog by KEPT_AGENTS.
  ...
  return catalog.filter((entry) => isForkKeptAgent(entry.id))
})
